...
var color = "BlueViolet" setget _color
var Nick = ""
func _color(text):
	chat("/color " + text)
...
const Colors = { "Blue":"", "BlueViolet":"", "CadetBlue":"","Chocolate":"", "Coral":"", "DodgerBlue":"", "Firebrick":"", "GoldenRod": "#DAA520", "Green":"", "HotPink":"", "OrangeRed":"", "Red":"", "SeaGreen":"", "SpringGreen":"", "YellowGreen": "#DAA520"}
...
func disconnect_from_twitch() -> void:
	if(websocket.disconnect_from_host() != OK):
		print_debug("Could not disconnect from Twitch.")
...
func chat(message : String, channel : String = ""):
	emit_signal("chat",message,[Colors[color],Nick])
...
func add_command(cmd_name : String, instance : Object, instance_func : String, max_args : int = 0, min_args : int = 0, permission_level : int = PermissionFlag.EVERYONE, where : int = WhereFlag.CHAT,remember = false) -> void:
	var func_ref = FuncRef.new()
	func_ref.set_instance(instance)
	func_ref.set_function(instance_func)
	commands[cmd_name] = CommandData.new(func_ref, permission_level, max_args, min_args, where,false)
...
func get_original_command(cmd_name : String):
	var to_ = commands.get(cmd_name)
	if(to_):
		if !to_.alias:
			print("returned")
			return
		for command in commands.keys():
			if(commands[command].func_ref == to_.func_ref and !commands[command].alias):
				return command
		print("not found")

func add_alias(cmd_name : String, alias : String) -> void:
	if(commands.has(cmd_name)):
		var c = commands.get(cmd_name)
		c = CommandData.new(c.func_ref,c.permission_level,c.max_args,c.min_args,c.where)
		commands[alias] = c
...
func handle_message(message : String, tags : Dictionary) -> void:
	...
	match msg[1]:
		...
		"PRIVMSG":
			...
			_msg.remove(0)
			_msg.remove(0)
			_msg[0] = _msg[0].right(1)
			var n = 0
			while n < _msg.size():
				_msg[n] = _msg[n].to_lower()
				n += 1
			handle_command(sender_data, msg)
			
			emit_signal("chat_message", sender_data, _msg.join(" "))
			if(get_images):
		...
		"JOIN":
			emit_signal("user_joined",msg[0].right(1).split("!",1)[0])
		_:
...
func handle_command(sender_data : SenderData, msg : PoolStringArray, whisper : bool = false) -> void:
	if(command_prefixes.has(msg[3].substr(1, 1))):
		var command : String  = msg[3].right(2).to_lower()
		...
		if(cmd_data):
			...
			var n = 0
			while n < arg_ary.size():
				arg_ary[n] = arg_ary[n].to_lower()
				n += 1
